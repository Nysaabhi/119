...
function showLocatorsOverlay() {
  ...
  // Get the currently selected metropolis from localStorage or default to 'All'
  const selectedMetropolis = localStorage.getItem('selectedMetropolis') || 'All';

  // Filter locators based on the selected metropolis
  let filteredLocators = locatorsData;
  if (selectedMetropolis !== 'All') {
    filteredLocators = locatorsData.filter(locator => locator.metropolis === selectedMetropolis);
  }

  // Get unique categories for the selected metropolis
  const categories = ['All', ...new Set(filteredLocators.map(loc => loc.category))];
  ...
  overlay.innerHTML = `
    <div class="locators-content">
      ...
      <div class="locators-grid" id="locatorsGrid">
        ${renderLocatorCards(filteredLocators)}
      </div>
    </div>
...
// Function to handle metropolis selection
...
// Function to filter locators based on category and metropolis
